...
class RegisterViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    @IBAction func registerButtonPressed(_ sender: UIButton) {
        ...
        // Mevcut müşteri kontrolü yapalım
        checkIfCustomerExists(email: email) { exists in
            if exists {
                DispatchQueue.main.async {
                    self.showAlert(message: "Bu email ile daha önce kullanıcı oluşturulmuş.")
                }
            } else {
                let selectedPackage = self.packages[selectedRow]
                ...
                self.registerUser(with: parameters)
            }
        }
    }
    
    func checkIfCustomerExists(email: String, completion: @escaping (Bool) -> Void) {
        guard let url = URL(string: "http://35.242.205.201/v1/api/customer/getAllCustomers") else {
            print("Invalid URL.")
            completion(false)
            return
        }
        ...
        let task = URLSession.shared.dataTask(with: url) { data, response, error in
            ...
            do {
                if let customers = try JSONSerialization.jsonObject(with: data, options: []) as? [[String: Any]] {
                    for customer in customers {
                        if let existingEmail = customer["email"] as? String, existingEmail == email {
                            completion(true)
                            return
                        }
                    }
